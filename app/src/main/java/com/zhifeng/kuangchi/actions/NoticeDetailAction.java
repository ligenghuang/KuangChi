 ...
 * 公告详情页
 */
public class NoticeDetailAction extends BaseAction<NoticeDetailView> {
    public NoticeDetailAction(RxAppCompatActivity _rxAppCompatActivity,NoticeDetailView view) {
                ...
                switch (action.getIdentifying()) {
                    case WebUrlUtil.POST_GET_ANNOUNCE_DETAIL:
                        ...
                        if (aBoolean) {
                            ...
                            NoticeDetailDto noticeDetailDto = new Gson().fromJson(action.getUserData().toString(), new TypeToken<NoticeDetailDto>() {
                            }.getType());
                            if (noticeDetailDto.getStatus() == 200){
                                //todo 获取首页数据成功
                                view.getNoticeDetailSuccess(noticeDetailDto);
                                return;
                            }
                            view.onError(noticeDetailDto.getMsg(),action.getErrorType());
                            return;
                        }
